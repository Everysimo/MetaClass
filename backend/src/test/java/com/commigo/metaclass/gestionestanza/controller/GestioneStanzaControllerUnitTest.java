...
class GestioneStanzaControllerUnitTest {
   ...
   */
  @MockBean private GestioneStanzaService stanzaService;

  @MockBean private ValidationToken validationToken;
  @MockBean private JwtTokenUtil jwtTokenUtil;
  @MockBean private StanzaRepository stanzaRepository;
  @InjectMocks private GestioneStanzaControl stanzaController;
  ...
  private ResultActions sendRequestStanza(Stanza stanza) throws Exception {
    ...
    return MockMvcBuilders.standaloneSetup(stanzaController).build().perform(requestBuilder);
  }

  // metodo che produce una richiesta testa un errore 500 durante la richiesta
  private void sendRequestFailureAccessoStanza(
      Stanza stanza, Class<? extends Exception> exceptionClass, int status) throws Exception {
    // Formattamento della richiesta
    MockHttpServletRequestBuilder requestBuilder =
    ...
    try {
      ...
      doThrow(exceptionClass).when(stanzaService).accessoStanza(any(), any());

      // Esecuzione della richiesta e ritorno della risposta
      MvcResult result =
          MockMvcBuilders.standaloneSetup(stanzaController)
              .build()
  ...
  private void sendRequestFailureCreaStanza(
      Stanza stanza, Class<? extends Exception> exceptionClass, int status) throws Exception {
    // Formattamento della richiesta
    MockHttpServletRequestBuilder requestBuilder =
        MockMvcRequestBuilders.post(API_URL_CREASTANZA)
    ...
    try {
      // Forza il metodo creaStanza a lanciare un'eccezione ServerRuntimeException
      doThrow(exceptionClass).when(stanzaService).creaStanza(any(), any());
  ...
  /**
   * testing crea stanza per ogni test case vado a richiamare un metodo che valida delle stanze
   * volontariamente errate per testare i messaggi di errori che vengono ritornati successivamente
   * si controlla ogni istruzione del metodo
   */
  @Test
  ...
  public void testCreaStanzaOnTestCase11() throws Exception {
    ...
    try {
      ...
      // ritorno della risposta
      ResultActions ra =
          MockMvcBuilders.standaloneSetup(stanzaController).build().perform(requestBuilder);
  ...
  /**
   * testing accesso stanza per ogni test case vado a richiamare un metodo che valida delle stanze
   * volontariamente errate per testare i messaggi di errori che vengono ritornati successivamente
   * si controlla ogni istruzione del metodo
   */
  @Test
